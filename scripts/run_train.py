...
from ntc import get_device, load_image, psnr, raw_bytes, save_image  # noqa: E402
...
def main() -> int:
    ...
    print(f"texture {args.texture}  size {args.size}  {width}x{height}")
    ...
    save_image(hstack([texture, recon], gap=6),
               out / f"{args.texture}_{args.size}.png")
    ...
    ax.set_title(f"Training {args.size} on {args.texture}")
    ax.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(out / f"{args.texture}_{args.size}_psnr.png", dpi=150)
    print(f"wrote {out.relative_to(ROOT)}/{args.texture}_{args.size}.png and _psnr.png")
    return 0
